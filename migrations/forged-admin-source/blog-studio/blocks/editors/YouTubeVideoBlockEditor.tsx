...
type Props = { block: YouTubeVideoBlock; onChange: (b: YouTubeVideoBlock) => void }

export default function YouTubeVideoBlockEditor({ block, onChange }: Props) {
  const d = block.data
  const [url, setUrl] = useState(d.url ?? '')
  const videoId = extractYouTubeId(url)

  function update(patch: Partial<YouTubeVideoBlock['data']>) {
  ...
  return (
    <div className="p-4 space-y-4">
      <div>
        <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1.5">YouTube URL <span className="text-red-500">*</span></label>
        ...
        {videoId
          ? <p className="mt-1 text-xs text-red-600 bg-red-50 border border-red-200 px-2.5 py-1 rounded-lg inline-block font-semibold">YouTube detected ✓ — ID: {videoId}</p>
          : url
            ? <p className="mt-1 text-xs text-orange-600">⚠ Not a recognized YouTube URL</p>
            : <p className="mt-1 text-xs text-gray-400">Supports: youtube.com/watch?v=, youtu.be/, /embed/, /shorts/</p>
        }
      </div>
      ...
      {videoId && (
        <div>
          ...
          <div className="aspect-video rounded-xl overflow-hidden bg-black shadow">
            <iframe
              src={buildYouTubeEmbedUrl(videoId, { autoplay: d.autoplay, loop: d.loop })}
      ...
      <div>
        ...
        <input type="text" value={d.heading ?? ''} onChange={(e) => update({ heading: e.target.value })}
          ...
          placeholder="e.g. Watch Our Suzuki Outboard Walkthrough" />
      </div>
      <div>
